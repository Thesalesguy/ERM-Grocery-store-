...
### 2.7 CI-only failure: hardcoded `created_by=1` in two idempotency tests (test-authoring bug, LOW — no production code affected)
`test_service_layer_retry_with_same_key_returns_original` and `test_concurrent_duplicate_adjustment_requests_create_only_one_adjustment` (both in `tests/test_inventory_idempotency.py`, Session B) called `create_stock_adjustment` directly with a hardcoded `created_by=1`, instead of creating a real user via `make_user_with_role` as every other test in the same file does. This passed in the local sandbox (a leftover user with id 1 existed there) but failed deterministically in CI's clean database with `ForeignKeyViolation: Key (created_by)=(1) is not present in table "users"` — confirmed via `mcp__github__get_job_logs` against workflow run `35728561735` (`2 failed, 892 passed`). Root-caused from the actual CI traceback, not guessed. Fixed by creating a real `INVENTORY_CLERK` user with `make_user_with_role` in both tests (and threading its id through `_attempt_adjustment` for the concurrency test) and using `user.id` instead of the literal `1`. No application code was touched. Re-verified: the two tests pass individually, the full `test_inventory_idempotency.py` (5/5) passes, and the full backend suite (894/894) passes locally against real PostgreSQL after the fix.
